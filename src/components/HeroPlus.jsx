...
const togglePlay = () => {
    const nextPlayingOption = !isPlaying;
    setIsPlaying(nextPlayingOption);
    sendCommand(nextPlayingOption ? 'playVideo' : 'pauseVideo');
};

const toggleMute = () => {
    const nextMuteOption = !isMuted;
    setIsMuted(nextMuteOption);
    sendCommand(nextMuteOption ? 'mute' : 'unMute');
};
                    ...
                    <div id="hero-video-container" className="w-full max-w-xl aspect-video relative rounded-2xl overflow-hidden shadow-2xl shadow-emerald-900/20 border border-slate-800/50 bg-black group z-20">
                        ...
                        <div className="absolute inset-0 pointer-events-none" style={{ transform: 'scale(1.3)' }}>
                            <iframe
                                ...
                                src="https://www.youtube.com/embed/b0l5aMp5fHc?enablejsapi=1&autoplay=1&mute=1&controls=0&modestbranding=1&showinfo=0&rel=0&loop=1&playlist=b0l5aMp5fHc&playsinline=1"
                                title="ITR Video"
                                frameBorder="0"
